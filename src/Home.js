 ...
 function Home() {
  ...
  function Word({ children, ...props }) {
    ...
    function handleNavigate() {
      ...
      if (children == "Products") navigate("../products");
      if (children == "Gallery") navigate("../gallery");
  ...
  function Cloud({ count = 4, radius = 20 }) {
    ...
    const words = useMemo(() => {
      ...
      temp.push([new THREE.Vector3().setFromSpherical(spherical.set(radius, phiSpan * 1, thetaSpan * 0)), 'History'])
      temp.push([new THREE.Vector3().setFromSpherical(spherical.set(radius, phiSpan * 1, thetaSpan * 1)), 'Foundation', 'news'])
      temp.push([new THREE.Vector3().setFromSpherical(spherical.set(radius, phiSpan * 1, thetaSpan * 2)), 'Gallery'])
      temp.push([new THREE.Vector3().setFromSpherical(spherical.set(radius, phiSpan * 2, thetaSpan * 3)), 'FriedrichRapp'])
